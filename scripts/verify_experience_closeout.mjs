...
(async()=>{
 ...
 for(const [domain,...scenarios] of cases){
  ...
  for(const scenario of scenarios){
   ...
   const workflow=page.locator('.experience-step').filter({hasText:'流程编排'});
   assert.equal(await workflow.isDisabled(),true,'fresh scenario requires submission');
   ...
   await page.goto(`${base}/domains/${domain}/scenarios?scenario=${scenario}&step=workflow`);
   await page.locator('.operator-item').first().waitFor();
   await page.waitForURL(url=>url.searchParams.get('step')==='operator');
   assert.equal(new URL(page.url()).searchParams.get('step'),'operator');
  }
 ...
 const row=page.locator('.exp-monitor .el-table__row').filter({hasText:runId});
 await row.getByRole('button',{name:'查看工作流',exact:true}).click();
 await page.waitForURL(`**/runs/${runId}`);await page.locator('.vue-flow__node').first().waitFor();
 await page.goBack();await page.locator('.experience-flow:not([inert])').waitFor();
