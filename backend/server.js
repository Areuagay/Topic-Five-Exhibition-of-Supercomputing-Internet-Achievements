...
import { createExperienceStore, EXPERIENCE_DOMAINS } from './experience-store.js'
...
// Runtime changes are persisted separately from versioned demonstration data.
const experienceStore = createExperienceStore({
  dataDir: MOCK_DATA_DIR,
  runtimeDir: process.env.MOCK_RUNTIME_DIR || resolve(__dirname, '..', '.runtime-data'),
})
...
const server = createServer(async (req, res) => {
  ...
  if (method === 'GET') {
    ...
    if (EXPERIENCE_DOMAINS.includes(parts[0]) && parts[1] === 'upload-samples') {
      sendJSON(res, 200, {
        code: 200,
        message: 'success',
        data: (existsSync(resolve(UPLOADS_DIR, parts[0])) ? readdirSync(resolve(UPLOADS_DIR, parts[0])).map(file => ({ file_name: file, dataset_id: file.split('.')[0], format: extname(file).slice(1).toUpperCase(), size_bytes: statSync(resolve(UPLOADS_DIR, parts[0], file)).size })) : []),
        timestamp: new Date().toISOString(),
      })
      return
    }

    const live = experienceStore.read(parts)
    if (live) {
      sendJSON(res, live.code, { ...live, timestamp: new Date().toISOString() })
      return
    }
  ...
  const experienceResult = experienceStore.write(method, writeParts, body)
  if (experienceResult) {
    sendJSON(res, experienceResult.code || 200, {
      code: experienceResult.code || 200,
      message: experienceResult.message || 'success',
      data: experienceResult.data,
      timestamp: new Date().toISOString(),
    })
    return
  }
